...
def test_decimal_places(answer_path, files, cols):
    ...
    # TODO: this logic is flawed when filelist is more than 1 file long.
    for i, ans in enumerate(filelist):
        ...
        assert answer_key_to_check.shape == answer_student_to_check.shape, f"Rows for {ans} answer key ({answer_key_to_check.shape[0]}) not equal to submission shape ({answer_student_to_check.shape[0]})"

        # Find where errors exist, and identify the rows and columns of interest.
        df_diff = abs(answer_key_to_check - answer_student_to_check)
        df_below_delta = df_diff <= ATOL
        ...
        result = answer_key[~df_below_delta.all(axis=1)]
